    ...
    public class NetworkFolderCopier
    {
            ...
            string password = null)
        {
            ...
            if (string.IsNullOrWhiteSpace(PN))
                throw new ArgumentException("搜索关键词不能为空", nameof(PN));
            ...
            try
            {
                ...
                foreach (string dirPath in topLevelDirs)
                {
                    ...
                    if (dirName.IndexOf(PN, StringComparison.OrdinalIgnoreCase) >= 0)
                    {
                        foundMatch = true;
                        Log($"\n? 找到匹配的一级文件夹: {dirName}");
                        Log($"   路径: {dirPath}");
                        ...
                        foreach (string subFolder in subFolders)
                        {
                            ...
                            if (folderName.IndexOf(ON, StringComparison.OrdinalIgnoreCase) >= 0)
                            {
